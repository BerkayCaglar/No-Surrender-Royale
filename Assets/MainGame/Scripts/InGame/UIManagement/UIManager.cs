...
public class UIManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _downloadMenu, _downloadingMenu, _downloadedMenu, _loadingMenu, _checkingForUpdatesMenu, _battleSceneMainCanvas;
    ...
    #region Card Sockets
    [SerializeField] private List<GameObject> _cardSockets = new List<GameObject>();

    public List<GameObject> CardSockets
    {
        get => _cardSockets;
    }
    ...
    public void PlayButton()
    {
        ...
        // Start the countdown and spawn coroutine
        //StartCountdownAndSpawnCoroutine();
    }
    ...
    #region Waiting For Players Methods

    public void ShowWaitingForPlayersMenu()
    {
        _playerMainPanel.SetActive(false);
        _waitingForPlayersPanel.SetActive(true);
    }

    public void HideWaitingForPlayersMenu()
    {
        _playerMainPanel.SetActive(true);
    ...
    #region Photon Bolt Server Methods

    public void StartCountdownAndSpawnCoroutine()
    {
    ...
    public void ConfigureTheServerRpc(Vector3 _cameraPosition, Quaternion _cameraRotation)
    {
        // Close the battle scene canvas
        _battleSceneMainCanvas.SetActive(false);

        // Set the CameraPositionAndRotation to the GameManager
        GameManager.Instance.SetCameraPositionAndRotation(_cameraPosition, _cameraRotation);
    ...
    public void ConfigureTheClientRpc(Vector3 _cameraPosition, Quaternion _cameraRotation)
    {
        // Set the CameraPositionAndRotation to the GameManager
        GameManager.Instance.SetCameraPositionAndRotation(_cameraPosition, _cameraRotation);
    ...
    public void SetPlayerCards(bool _isFirst)
    {
        // If the player is the first player, set the cards to the first player cards
        if (_isFirst)
        {
            for (int i = 0; i < _cardSockets.Count; i++)
            {
                GameObject _card = GameManager.Instance.Player1Deck[i];

                _card.gameObject.GetComponent<DraggableItem>()._sideName = "PlayerSide";

                GameObject _instantiatedCard = Instantiate(_card, Vector3.zero, Quaternion.identity);

                _instantiatedCard.transform.SetParent(_cardSockets[i].transform);

                _instantiatedCard.transform.localScale = new Vector3(0.9f, 0.9f, 0.9f);
            }
        }
        // If the player is the second player, set the cards to the second player cards
        else
        {
            for (int i = 0; i < _cardSockets.Count; i++)
            {
                GameObject _card = GameManager.Instance.Player2Deck[i];

                _card.gameObject.GetComponent<DraggableItem>()._sideName = "EnemySide";
